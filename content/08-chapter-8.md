...
<p class="box-title"><em>Werner state with fidelity F</em></p>

</div>

The Werner state is a convex mixture of the maximally entangled Bell state |Φ^+⟩ (with weight F) and the completely mixed state I\_4/4 (with weight 1−F). It arises naturally when a Bell pair passes through a depolarising channel ε with depolarisation probability p: the output fidelity is F = 1 − (3/4)p. Werner states can be distilled by BBPSSW if and only if F > 1/2.

### 8.2.1 Protocol Steps

The BBPSSW protocol requires Alice and Bob each to have two copies of ρ\_Werner (pairs labelled 1 and 2). It proceeds in three steps:

- Step 1 (Bilateral CNOT): Alice applies a CNOT gate with pair 1 as control and pair 2 as target on her side. Bob does the same with his qubits. This 'bilateral CNOT' correlates the error syndrome of the two pairs.

- Step 2 (Measurement): Alice measures her qubit from pair 2 in the Z-basis and obtains result m\_A ∈ {0,1}. Bob does likewise on his qubit from pair 2 and obtains m\_B.

- Step 3 (Classical comparison): Alice and Bob communicate their measurement results via the classical channel. If m\_A = m\_B (the results agree), pair 1 has been successfully distilled to higher fidelity and is kept. If m\_A ≠ m\_B, pair 1 is discarded.

<div class="box box-generic">

<img class="fig-img" src="content/images/image59.png" alt="">
</div>

**Figure 8.1: BBPSSW Entanglement Distillation Protocol and Fidelity Curves**

Left: The BBPSSW protocol circuit for one distillation round. Alice (top) and Bob (bottom) each start with two copies of Werner state ρ\_F. Alice applies CNOT(pair1, pair2), Bob applies CNOT(pair1, pair2). Both measure pair 2 in Z-basis and compare results via classical channel (orange arrow). If equal (success, probability P\_s), pair 1 is retained with improved fidelity F'. Right: Fidelity improvement curves. Blue curve: BBPSSW output fidelity F' vs. input fidelity F for Werner states. Green curve: DEJMPS protocol. The diagonal dashed line marks F\_out = F\_in (no improvement). Both protocols improve fidelity for F > 0.5. The gold marker shows the worked example: F = 0.75 → F' = 0.90. Multiple rounds (shown by arrows) converge toward F = 1.

### 8.2.2 BBPSSW Fidelity Formula Derivation

To derive the output fidelity, we track how the bilateral CNOT correlates the two Werner states. The key observation: for a Werner state, the error part I\_4/4 can be decomposed as a uniform mixture of the four Pauli errors (I, X, Y, Z) applied to one qubit. The bilateral CNOT creates a correlation such that: if both pairs have the same Pauli error (or both are error-free), the measurement results agree (m\_A = m\_B). If the pairs have different Pauli errors, the results disagree.

More precisely, after the bilateral CNOT:

- Pair 1 is error-free AND pair 2 is error-free → m\_A = m\_B = 0 (success); pair 1 remains error-free.

- Pair 1 has an X error AND pair 2 has an X error → m\_A = m\_B = 1 (success); the X errors on pair 1 and pair 2 propagate through CNOT and cancel. The retained pair 1 is error-free!

- Pair 1 is error-free AND pair 2 has an X error → m\_A ≠ m\_B (failure); pair 1 is discarded.

- Other Pauli error combinations contribute to both success and failure cases.

For Werner states, the probability of success (m\_A = m\_B) and the conditional fidelity of the retained pair 1 work out to:

| F' = F² / (F² + (1−F)²) | BBPSSW output fidelity |
|---|---|
|  | BBPSSW success probability |
...
- Step 1: Each party applies a local Y-rotation (by π/4) to their qubit from pair 2, transforming the error basis.

- Step 2: Bilateral CNOT, as in BBPSSW.

- Step 3: Both parties measure pair 2 in the X-basis (instead of Z-basis as in BBPSSW) and compare results.
...
<div class="box box-generic">
<p class="box-title"><em>Bell-diagonal state (most general two-qubit mixed state in the Bell basis)</em></p>

</div>

For Bell-diagonal states, the DEJMPS output fidelity formula is more complex but can be computed analytically. For Werner states specifically (where λ\_1 = λ\_2 = λ\_3 = (1−F)/4), DEJMPS and BBPSSW give comparable performance. DEJMPS shows a significant advantage over BBPSSW for Bell-diagonal states where λ\_2 (the |Ψ^+⟩ component) is large, as can arise from coherent errors in the quantum channel.
...
A single round of BBPSSW increases fidelity from F to F', consuming 2 pairs to produce 1 pair with probability P\_success. To approach F → 1, multiple rounds are required. After k rounds of BBPSSW, the protocol consumes 2^k initial pairs to produce one pair with probability P\_total = ∏\_{i=1}^{k} P\_{success}^{(i)}.

The resource cost grows exponentially with the number of rounds: 2 pairs per round, so 2^k pairs consumed after k rounds. This is acceptable for small k (2–3 rounds is practical) but becomes prohibitive for large k. The practical question is: how many rounds are needed to reach a target fidelity F\_target, given a starting fidelity F\_0?

Iterated application of the BBPSSW map: F\_1 = F\_0^2/(F\_0^2+(1-F\_0)^2), F\_2 = F\_1^2/(F\_1^2+(1-F\_1)^2), etc. This converges monotonically to F = 1 as k → ∞ for any starting F\_0 > 0.5.

<div class="box box-generic">
<p class="box-title">k_rounds ≈ log_2[log_{F′/F}(F_{target}/(1-F_{target}))] + const</p>
<p><em>Approximate rounds needed (see derivation in Theory Question 8.4)</em></p>
</div>
...
<img class="fig-img" src="content/images/image60.png" alt="">
</div>
...
- China (2017–2021): The 2000 km Beijing–Shanghai backbone, operated by QuantumCTek, uses ~32 trusted relay stations and delivers AES-256-encrypted commercial services. Extended in 2021 to a 4600 km integrated space-ground network incorporating the Micius satellite.

- Europe (2022–present): The OpenQKD consortium (10 countries) and the EuroQCI (European Quantum Communication Infrastructure) programme, targeting a pan-European quantum-secured network by 2027.

- South Korea: SK Telecom commercial QKD network integrated into their 5G backbone, serving banking and government clients.

- Japan: NTT-Toshiba consortium quantum communications network for data centre interconnects in Tokyo.

- India (2022–present): IIT Delhi and DRDO have deployed a 100 km QKD link in the Delhi metropolitan area on existing fibre infrastructure. C-DOT (Centre for Development of Telematics) has demonstrated 20 km free-space QKD on the Deccan Plateau with indigenous BB84 hardware.
...
Hardware requirements for Stage 2 transition: quantum memories with T\_2 >> classical link latency (T\_2 > 10 ms for 100 km segments); BSM efficiency > 50%; entanglement distillation capability; heralded entanglement generation with fidelity F\_raw > 0.7 per segment. These are technically challenging but achievable with current NV-centre and AFC rare-earth memory technology.
...
The fundamental limitation of ground-based fibre QKD networks is the exponential photon loss: η\_fibre(L) = 10^{−αL/10}. Even with quantum repeaters, the engineering complexity and cost of deploying quantum memories at every relay station over thousands of kilometres is enormous. Satellite-based QKD offers a fundamentally different approach: instead of propagating quantum signals through kilometres of glass, route them through space, where the dominant loss mechanism is geometric beam divergence rather than material absorption.

### 8.6.1 Why Satellites? The Free-Space Loss Scaling Advantage

The transmission of a photon beam from a satellite at altitude h to a ground receiver involves two loss mechanisms: geometric (diffraction) loss and atmospheric loss. For a transmitter aperture d\_T, wavelength λ, and altitude h, the beam radius at the ground is r = 2.44λh/d\_T (Rayleigh criterion). The geometric efficiency is:

<div class="box box-generic">
<p class="box-title">η_geo = (π d_R^2/4) / (π r^2) = (d_R/2r)^2 = (d_R d_T / (2.44λh))^2</p>
<p><em>Satellite geometric channel efficiency</em></p>
</div>

The key insight: η\_geo scales as h^{-2} (quadratic in distance), whereas η\_fibre scales as e^{−αh} (exponential). For large h (>200 km), quadratic loss is dramatically less severe than exponential loss. At h = 500 km, η\_geo is roughly equivalent to the fibre transmission over only ~15–20 km of fibre.

The atmospheric loss η\_atm is a fixed multiplicative factor (independent of orbital altitude) that accounts for absorption and scattering in the ~10 km thick atmosphere. Typical values: η\_atm ≈ 0.3–0.7 at night, 0.1–0.4 during daytime (due to solar background photons increasing the noise). The total satellite channel efficiency is:

<div class="box box-generic">
<p class="box-title">η_satellite = η_geo × η_atm × η_pointing</p>
<p><em>Total satellite channel efficiency</em></p>
</div>

The pointing efficiency η\_pointing accounts for imperfect tracking: the satellite must track the ground station to < 0.5 μrad accuracy during a 100-second overhead pass. This requires active tip-tilt mirrors and gyroscopic stabilisation systems.
...
<img class="fig-img" src="content/images/image61.png" alt="">
</div>
...
| Country/Agency | Programme | Satellites | Target Date | Capability |
|---|---|---|---|---|
| China | QSS-2 / SQSS | 6+ LEO | 2025–2028 | 24/7 global QKD constellation |
| Europe (ESA) | SAGA | 2 MEO | 2026–2028 | Quantum-secured satellite comm. demo |
| UK | UKspace-QKDN | 2 LEO | 2026–2028 | Satellite QKD for UK government |
| USA | DOE/NSF | Multiple | 2027–2030 | Quantum network testbed in space |
| India (NQM) | Q-Sat | 1 LEO | 2026–2028 | Satellite QKD and entanglement demo |
...
<img class="fig-img" src="content/images/image62.png" alt="">
</div>
...
| Institution | Role | Key Achievement |
|---|---|---|
| IIT Delhi + DRDO | QKD link deployment, repeater R&D | 100 km QKD on deployed metro fibre (2022); BB84 with SNSPD, 1 Mbps key rate |
| C-DOT (Telematics) | Indigenous QKD hardware | First indigenous BB84 system; 20 km free-space QKD (Deccan Plateau) |
| TIFR Mumbai | Entanglement sources, photonics | Integrated photonic QKD chips; entangled photon sources at 810 nm |
| IISc Bangalore | Quantum repeater components | Rare-earth crystal AFC memories; entanglement distillation protocols |
| IIT Bombay | Photonic chip QKD | On-chip silicon photonics QKD transmitter (1550 nm) |
| ISRO (future) | Quantum satellite (Q-Sat) | Planned satellite QKD and entanglement demo by 2026–2028 |
| RBI + SEBI | QRNG for banking | Pilot deployment of QRNG in Mumbai securities exchange (2023) |
...
| Phase | Timeline | Distance | Milestones |
|---|---|---|---|
| Pilot (Delhi–Agra) | 2025–2026 | 200 km | QKD on RailTel OFC; 10 kbps key rate; first operational test of NQM infrastructure |
| Phase 1 (Delhi–Nagpur) | 2026–2027 | 800 km | 5 trusted relay stations; SNSPD deployed; 2 kbps end-to-end key rate |
| Phase 2 (Full link) | 2027–2028 | 1400 km | Full Delhi–Pune-Mumbai corridor; 15 relays; satellite backup via Q-Sat |
| Phase 3 (National backbone) | 2029–2031 | 2000+ km | Five-city ring (Delhi–Kolkata–Chennai–Bangalore–Mumbai–Delhi); Stage 2 pilot |
...
Satellite quantum communication uses satellites to transmit quantum states or entangled photons between distant ground stations through free-space optical links.

**Answer 2:**

Satellites are useful because free-space loss scales more favourably than fibre loss, enabling quantum communication over thousands of kilometres.

**Answer 3:**

Geometric diffraction loss occurs because an optical beam spreads as it propagates through space, reducing the fraction of photons collected by the receiver.

**Answer 4:**

Quantum Key Distribution is a method of securely sharing cryptographic keys using quantum mechanical principles.

**Answer 5:**

The Micius satellite demonstrated long-distance entanglement distribution, satellite QKD, and quantum teleportation over record distances.

**Answer 6:**

Quantum teleportation is the transfer of an unknown quantum state from one location to another using shared entanglement and classical communication.

**Answer 7:**

In a trusted-node architecture, the satellite temporarily stores or processes key information and must therefore be trusted by communicating parties.

**Answer 8:**

In uplink communication, photons travel from ground to satellite, while in downlink communication photons travel from satellite to ground.

**Answer 9:**

Atmospheric turbulence causes beam wandering, scattering, and phase fluctuations, reducing communication fidelity and transmission efficiency.

**Answer 10:**

Entangled photon sources generate correlated photon pairs required for entanglement distribution, teleportation, and entanglement-based QKD.

**Answer 11:**

Classical communication transmits the measurement results needed for the receiver to reconstruct the original quantum state during teleportation.

**Answer 12:**

The quantum internet is a global network capable of distributing entanglement and enabling quantum communication and distributed quantum computing.

**Answer 13:**

Major challenges include photon loss, decoherence, limited quantum memory, precise synchronisation, and scalable quantum repeater technology.

**Answer 14:**

Quantum network synchronisation ensures that photon generation, transmission, and detection occur at precisely coordinated times across the network.

**Answer 15:**

Satellite-based QKD can achieve global-scale communication with lower effective loss compared to very long optical fibres.
...
(c) Number of successful distillation attempts = (1000 pairs / 2) × P\_success = 500 × 0.68 = 340 pairs
...
(c) P\_total = P\_0 × P\_1 × P\_2 × P\_3 = 0.58 × 0.738 × 0.938 × 0.999 = 0.402
...
(a) Beam half-angle: θ = 2.44λ/d\_T = 2.44×785×10^{-9}/0.3 = 6.38×10^{-6} rad = 6.38 μrad

Beam radius at ground: r = θ × h = 6.38×10^{-6} × 500×10^3 = 3.19 m

(b) η\_geo = (d\_R/2r)^2 = (0.6/(2×3.19))^2 = (0.094)^2 = 8.85×10^{-3} = 0.885%

(c) η\_total = η\_geo × η\_atm × η\_pointing = 0.00885 × 0.5 × 0.7 = 3.1×10^{-3} = 0.31%
...
(a) The maximum quantum mechanical value of the CHSH parameter (Tsirelson bound) is S\_max = 2√2 ≈ 2.828.
...
(b) Signal count rate = μ × η\_fibre × η\_det × R\_source = 0.5 × 10^{-4} × 0.9 × 10^9 = 45,000 counts/s

(c) Dark count contribution: d × R = 10^{-7} × 10^9 = 100 dark counts/s

QBER\_dark = 100/(2 × 45,000) = 0.11% (small, dark count dominated at short distance)

(d) H\_2(0.0111) = -0.0111×log\_2(0.0111) - 0.9889×log\_2(0.9889) ≈ 0.089

R\_key = 0.5 × (1 - 2×0.089) × 45,000 = 0.5 × 0.822 × 45,000 ≈ 18.5 kbps secure key
...
(a) Werner state eigenvalues: λ\_1 = F+(1-F)/4 = F+(1-F)/4; wait, Werner state eigenvalues are:

Eigenvalues: λ\_1 = F + (1-F)/4 = (1+3F)/4, λ\_{2,3,4} = (1-F)/4

For F = 0.80: λ\_1 = (1+2.4)/4 = 0.85; λ\_{2,3,4} = 0.05
...
per-segment rate (since keys are relay-encrypted hop by hop): R\_segment = R\_e2e = 1 kbps.
...
Signal count rate: μ × η × η\_det × R = 0.5 × 3.16×10^{-3} × 0.9 × 10^9 = 1.42×10^6 counts/s

QBER ≈ 2% (alignment + dark counts); H\_2(0.02) ≈ 0.142

R\_key = 0.5 × (1-2×0.142) × 1.42×10^6 = 0.5 × 0.716 × 1.42×10^6 = 508 kbps
...
<div class="box box-generic">
<p class="box-title"><strong>1. The BBPSSW fidelity formula F' = F²/(F²+(1-F)²) applies to Werner states. The distillation threshold F &gt; 0.5 arises because:</strong></p>
<p>(A) Werner states with F &gt; 0.5 are the only ones that satisfy the no-cloning theorem</p>
<p>(B) Below F = 0.5 the Werner state is separable and has no distillable entanglement; above F = 0.5, the |Φ^+⟩ component is dominant and can be amplified by the bilateral CNOT protocol</p>
<p>(C) The bilateral CNOT gate introduces additional errors for F &lt; 0.5 that overwhelm any fidelity improvement</p>
<p>(D) Below F = 0.5 the success probability P_success exceeds 0.5, making distillation probabilistically unfavourable</p>
</div>

<div class="box box-generic">
<p class="box-title"><strong>2. Applying BBPSSW to F = 0.80, the output fidelity is:</strong></p>
<p>(A) F' = 0.80</p>
<p>(B) F' = 0.880</p>
<p>(C) F' = 0.941</p>
<p>(D) F' = 0.999</p>
...
<div class="box box-generic">
<p class="box-title"><strong>3. The Hashing bound for entanglement distillation states that:</strong></p>
<p>(A) The distillable entanglement E_D = 1 always, regardless of the noise level</p>
<p>(B) For large numbers of input copies, the maximum yield of Bell pairs per input copy is E_D = 1 − S(ρ), where S is the von Neumann entropy</p>
<p>(C) The Hashing bound sets a minimum fidelity for distillation to be possible</p>
<p>(D) E_D = F² for all Werner states, providing an upper bound on distillation yield</p>
</div>

<div class="box box-generic">
<p class="box-title"><strong>4. The DEJMPS protocol differs from BBPSSW in that it:</strong></p>
<p>(A) Uses a controlled-phase gate instead of a CNOT gate</p>
<p>(B) Measures pair 2 in the X-basis after applying a Y-rotation, allowing it to distill a broader class of Bell-diagonal states more efficiently than BBPSSW for states with large off-diagonal coherences</p>
<p>(C) Requires quantum error correction codes at each node to implement, unlike the simpler BBPSSW</p>
<p>(D) Produces two output pairs instead of one, doubling the yield of distillable entanglement</p>
</div>

<div class="box box-generic">
<p class="box-title"><strong>5. In the Wehner-Elkouss-Hanson three-stage quantum internet framework, device-independent QKD (DI-QKD) first becomes possible at:</strong></p>
<p>(A) Stage 1 (Trusted-Node QKD)</p>
<p>(B) Stage 2 (Entanglement Distribution Network)</p>
<p>(C) Stage 3 (Quantum Computing Network)</p>
<p>(D) DI-QKD is impossible at any stage of the quantum internet</p>
</div>

<div class="box box-generic">
<p class="box-title"><strong>6. The primary security limitation of Stage 1 trusted-node QKD networks is that:</strong></p>
<p>(A) Photon loss prevents QKD at distances exceeding 200 km even with trusted nodes</p>
<p>(B) Security depends on the physical security and integrity of every intermediate relay node; compromise of any relay node exposes the end-to-end key</p>
<p>(C) Trusted-node networks use classical encryption that is vulnerable to quantum computers</p>
<p>(D) The QKD rate at trusted nodes is too slow for practical communication applications</p>
</div>

<div class="box box-generic">
<p class="box-title"><strong>7. The Micius satellite achieved an effective link efficiency equivalent to approximately:</strong></p>
<p>(A) 500 km of optical fibre</p>
<p>(B) 15–20 km of optical fibre</p>
<p>(C) 1000 km of optical fibre</p>
<p>(D) The same as 1200 km of optical fibre</p>
</div>

<div class="box box-generic">
<p class="box-title"><strong>8. The geometric efficiency of a satellite QKD link scales as the satellite altitude h to the power:</strong></p>
<p>(A) h^{-1} (inverse linear)</p>
<p>(B) h^{-2} (inverse quadratic)</p>
<p>(C) e^{-h} (exponential decay)</p>
<p>(D) h^{-4} (inverse fourth power)</p>
</div>

<div class="box box-generic">
<p class="box-title"><strong>9. The Micius satellite’s Bell test (Yin et al., Science 2017) measured S = 2.37 over 1203 km. This violated the classical CHSH inequality because:</strong></p>
<p>(A) S = 2.37 is greater than the classical bound |S| ≤ 2, certifying that the distributed photon pairs were genuinely entangled and not describable by any local hidden variable theory</p>
<p>(B) S = 2.37 matches the quantum mechanical maximum exactly, proving the completeness of the Micius experiment</p>
<p>(C) Classical systems can achieve S = 2.37 but not S = 2.828, so the result is inconclusive</p>
<p>(D) The violation proves faster-than-light communication between the two ground stations</p>
</div>

<div class="box box-generic">
<p class="box-title"><strong>10. India’s National Quantum Mission quantum communications hub (QuCryptoS) is primarily tasked with:</strong></p>
<p>(A) Building the world’s fastest quantum computer using superconducting qubits</p>
<p>(B) Developing indigenous QKD systems, QRNG devices, and deploying a national quantum-secured communication network including the Delhi–Pune backbone</p>
<p>(C) Developing post-quantum cryptography standards to replace RSA and ECC encryption</p>
<p>(D) Deploying quantum sensors for geophysical mapping of India’s mineral resources</p>
</div>

<div class="box box-generic">
<p class="box-title"><strong>11. The preferred QKD encoding for long-distance Indian fibre links (Delhi–Pune corridor) is time-bin rather than polarisation because:</strong></p>
<p>(A) Time-bin QKD achieves higher key rates than polarisation-encoded QKD at all distances</p>
<p>(B) Time-bin encoding is immune to polarisation mode dispersion (PMD) that accumulates in deployed fibres, making it more robust for long-distance operation on existing infrastructure</p>
<p>(C) Time-bin photons travel faster in optical fibres than polarisation-encoded photons</p>
<p>(D) Polarisation encoding is prohibited under Indian telecommunications regulations</p>
</div>

<div class="box box-generic">
<p class="box-title"><strong>12. The QuTech three-node quantum network (Delft, 2021) was operating at approximately which stage of the quantum internet?</strong></p>
<p>(A) Early Stage 1 (trusted-node QKD)</p>
<p>(B) Early Stage 2 (genuine entanglement distribution)</p>
<p>(C) Stage 3 (blind quantum computing)</p>
<p>(D) It was a simulation and did not demonstrate real quantum operations</p>
</div>

<div class="box box-generic">
<p class="box-title"><strong>13. For the Delhi–Agra pilot QKD link (200 km), the dominant source of QBER at typical system parameters is:</strong></p>
<p>(A) Photon number splitting (PNS) attacks exploiting multi-photon pulses</p>
<p>(B) Optical alignment errors (≈1–2%) rather than dark counts (≈0.1%), making the link loss-limited but not dark-count limited</p>
<p>(C) Thermal photons in the fibre at 300 K that add background counts at the detector</p>
<p>(D) Quantum decoherence of photon polarisation in the glass fibre</p>
</div>

<div class="box box-generic">
<p class="box-title"><strong>14. The C-DOT (Centre for Development of Telematics) contribution to India’s quantum networking programme is significant because:</strong></p>
<p>(A) It is the only institution in India with quantum computers capable of breaking RSA-2048</p>
<p>(B) It developed the first indigenous Indian QKD system, demonstrating technological self-reliance and reducing dependence on imported quantum communication equipment from China or Europe</p>
<p>(C) It manages the entire 2000 km quantum fibre backbone and all relay stations under NQM</p>
<p>(D) It is responsible for India’s quantum satellite (Q-Sat) design and launch under ISRO</p>
</div>

<div class="box box-generic">
<p class="box-title"><strong>15. In the context of entanglement distillation, the term LOCC means:</strong></p>
<p>(A) Loss-Optimised Cavity-Coupled Communication, a method for enhancing photon collection efficiency in quantum memories</p>
<p>(B) Local Operations and Classical Communication: each party performs only quantum operations on their own qubits and communicates results via classical channels only, without sharing additional quantum states</p>
<p>(C) Long-range Optical Classical Communication, referring to the classical control channel in a quantum network</p>
<p>(D) Linear Optical Classical Computation, a technique used in post-quantum cryptography</p>
</div>
...
- Duan, L.-M., Lukin, M. D., Cirac, J. I., & Zoller, P. (2001). Long-distance quantum communication with atomic ensembles and linear optics. Nature, 414, 413–418. [DLCZ Protocol]

- Bennett, C. H., Brassard, G., Popescu, S., Schumacher, B., Smolin, J. A., & Wootters, W. K. (1996). Purification of noisy entanglement and faithful teleportation via noisy channels. Physical Review Letters, 76(5), 722. [BBPSSW]

- Deutsch, D., Ekert, A., Jozsa, R., Macchiavello, C., Palma, G. M., & Sanpera, A. (1996). Quantum privacy amplification and the security of quantum cryptography over noisy channels. Physical Review Letters, 77(13), 2818. [DEJMPS]

- Wehner, S., Elkouss, D., & Hanson, R. (2018). Quantum internet: A vision for the road ahead. Science, 362(6412), eaam9288.

- Sangouard, N., Simon, C., de Riedmatten, H., & Gisin, N. (2011). Quantum repeaters based on atomic ensembles and linear optics. Reviews of Modern Physics, 83(1), 33.
...
- Pompili, M. et al. (2021). Realization of a multinode quantum network of remote solid-state qubits. Science, 372(6539), 259–264. [QuTech 3-node network]
...
- Lago-Rivera, D. et al. (2021). Telecom-heralded entanglement between multimode solid-state quantum memories. Nature, 594, 37–41.

- Ma, Y. et al. (2021). One-hour coherent optical storage in an atomic frequency comb memory. Nature Communications, 12, 2381.

- Kómár, P. et al. (2014). A quantum network of clocks. Nature Physics, 10, 582–587.

- Ministry of Science and Technology, Government of India (2023). National Quantum Mission: Mission Document and Implementation Plan. Department of Science and Technology, New Delhi.

- Bhaskar, M. K. et al. (2020). Experimental demonstration of memory-enhanced quantum communication. Nature, 580, 60–64.

- IIT Delhi – DRDO Quantum Key Distribution Laboratory (2022). Demonstration of 100 km QKD link on deployed optical fibre infrastructure in the Delhi metropolitan area. Technical Report QTDL-2022-04.

- Broadbent, A., Fitzsimons, J., & Kashefi, E. (2009). Universal blind quantum computation. Proceedings of FOCS 2009, 517–526.

- Bennett, C. H. et al. (1996). Mixed-state entanglement and quantum error correction. Physical Review A, 54(5), 3824. [Hashing protocol]
